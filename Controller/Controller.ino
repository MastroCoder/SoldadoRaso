...
// Chamado toda vez que um novo controle é conectado. Pode conectar até 4 controles.
void onConnectedController(ControllerPtr ctl) {
  bool foundEmptySlot = false;
  for (int i = 0; i < BP32_MAX_GAMEPADS; i++) {
    if (myControllers[i] == nullptr) { // havendo espaço
...
// ========= Mostrador de valores do controle no serial ========= //
...
void processGamepad(ControllerPtr ctl) {
  ...
  if (ctl->axisY() <= -80) {
    // code for when left joystick is pushed up
      Serial.println("Robô estará indo para frente");
    }

  //== LEFT JOYSTICK - DOWN ==//
  if (ctl->axisY() >= 80) {
    // code for when left joystick is pushed down
    Serial.println("Robô estará indo para trás");
  }

  //== LEFT JOYSTICK - LEFT ==//
  if (ctl->axisX() <= -80) {
    // code for when left joystick is pushed left
    Serial.println("Robô estará indo para a esquerda");
  }

  //== LEFT JOYSTICK - RIGHT ==//
  if (ctl->axisX() >= 80) {
    // code for when left joystick is pushed right
    Serial.println("Robô estará indo para a direita");
  }

  //== LEFT JOYSTICK DEADZONE ==//
  if (ctl->axisY() > -80 && ctl->axisY() < 80 && ctl->axisX() > -80 && ctl->axisX() < 80) {
    // code for when left joystick is at idle
    Serial.println("Robô está idle.");
  }
